"""
Taken from the PyTorch tutorial on training neural network on MNIST
...
def main():
    ...
    parser.add_argument('--test-saved-model', action='store_true', default=False,
    ...
    if args.test_saved_model:
        stateDict = torch.load('mnist_cnn.pt', weights_only=True)
        model = Net().to(device)
